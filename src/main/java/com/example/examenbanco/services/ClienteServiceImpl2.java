...
@Service
@Primary
public class ClienteServiceImpl2 implements ClienteService {
    @Autowired
    ...
    public Mensaje retirar(long id, double cantidad) {
        Cliente cliente=repository.findById(id).get();
        Mensaje respuesta=new Mensaje(id,"IMPOSIBLE RETIRAR");
        if (cantidad<=cliente.getSaldo()) {
            cliente.setSaldo(cliente.getSaldo()-cantidad);
            repository.save(cliente);
            respuesta.setMensaje("Se retiro = "+cantidad+" de "+cliente.getSaldo());
        }
        return respuesta;
    }
